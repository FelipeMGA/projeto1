...
SETORES_FM = {"MATEUS", "TIAGO", "ROGER", "DAMILSON", "DIOGO", "RENATO", "JRNE 5N", "JRNE  5N", "JRNE 7D"}
...
def parse_args():
    ...
    global DRY_RUN, SKIP_EXPORT, SKIP_RENAME, PDF_DIR, EXCEL_PATH, MANUAL_CACHE_PATH
...
def solicitar_nome_manual(desc_original, code, sugestao):
    ...
    Se o usuário apertar ENTER, a sugestão automática é usada.
    """
...
def inserir_logotipo(ws):
    ...
    """
    setor = ws.title.upper()
    img_path = None

    if setor in SETORES_FM:
        for ext in ("png", "jpg", "jpeg"):
            p = Path(f"FM.{ext}")
            if p.exists():
                img_path = p
                break
    elif setor in SETORES_VISION:
        for ext in ("png", "jpg", "jpeg"):
            p = Path(f"VISION.{ext}")
            if p.exists():
                img_path = p
    ...
    if not img_path:
        print(f"[AVISO] Logo não encontrado para a aba {ws.title}. Esperado: FM.* ou VISION.*")
        return
